...
export async function start_kernel(context: ExtensionContext) {
  ...
  var w = new NodeWorker(
    path.join(context.extensionUri.fsPath, "out", "comlink.worker.js")
  );
  w.on("message", (message) => {
    message && "log" in message
      ? console.log(`> ${message.log} ${JSON.stringify(message.data)}`)
      : undefined;

    console.log("Message", message);
  });
  const nodeEndpoint = require("comlink/dist/umd/node-adapter.js");
  var remote = Comlink.wrap(nodeEndpoint(w));
